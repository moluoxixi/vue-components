import type { ConfigFormBreakpoint, ConfigFormResolvedLayout, ConfigFormResponsiveLayout, ConfigFormResponsiveLayoutOverride } from '../types'

export function resolveConfigFormNodeSpan(nodeSpan: number | undefined, layout: ConfigFormResolvedLayout): number {
...
function applyOverride(current: ConfigFormResolvedLayout, override: ConfigFormResponsiveLayoutOverride | undefined): ConfigFormResolvedLayout {
  return {
...
function normalizeLayoutValue(value: number | undefined, defaultValue: number): number {
  return typeof value === 'number' && Number.isFinite(value) ? Math.min(24, Math.max(1, Math.floor(value))) : defaultValue
}
