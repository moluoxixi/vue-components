  ...
  describe(`${provider.prefix} nested material integration`, () => {
    ...
    it('round-trips allowlisted props while preserving creation-only value scopes', () => {
      ...
      for (const setter of setters) {
        expect(setter.path[0]).toBe('props')
      ...
      expect(currentGraph().nodesById.orders).toMatchObject({
        valueScope: originalScope,
        props: { arrayDisplay: 'list', title: 'Invoice lines', readonly: true, disabled: true },
      })
      expect(() => createNodePathCommand(currentGraph(), 'home', ['orders'], ['valueScope', 'field'], 'order.details')).toThrow('DESIGNER_SETTER_PATH_FORBIDDEN')
      expect(() => createNodePathCommand(currentGraph(), 'home', ['orders'], ['bindings', 'value'], {})).toThrow('DESIGNER_SETTER_PATH_FORBIDDEN')
      expect(() => createNodePathCommand(currentGraph(), 'home', ['orders'], ['valueScope', '__proto__'], 'bad')).toThrow('DESIGNER_SETTER_PATH_FORBIDDEN')
      expect((currentGraph().nodesById.orders as LayoutNode).valueScope).toEqual(originalScope)
    })
