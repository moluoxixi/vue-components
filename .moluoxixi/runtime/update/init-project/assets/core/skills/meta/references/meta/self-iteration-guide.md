# Moluoxixi Self-Iteration Guide

How to maintain skill documentation when customizing Moluoxixi.

---

## Core Principle

**Every Moluoxixi modification MUST be documented in the appropriate skill.**

```
Modification to Moluoxixi → Update project-local (project skill)
Update to Moluoxixi itself → Update meta (meta skill)
```
...
```
Is this a modification to Moluoxixi?
│
├── YES: What kind?
│   │
...
│   │
│   ├── Bug fix to core Moluoxixi
│   │   └── Update ~/.claude/skills/meta/
│   │       (or project copy if reviewing first)
│   │
│   └── New feature to core Moluoxixi
│       └── Update meta after release
│
└── NO: Just using Moluoxixi
...
ls .claude/skills/project-local/SKILL.md

# If not, create it from template
mkdir -p .claude/skills/project-local
# Copy template from meta/references/project-local-template.md
```

### Step 2: Make the Moluoxixi Modification

Do your work: add command, modify hook, etc.

### Step 3: Document in Project Skill

Open `.claude/skills/project-local/SKILL.md` and:
...
```markdown
#### /moluoxixi:my-command
- **File**: `.claude/commands/moluoxixi/my-command.md`
...
**JSONL Integration**:
```jsonl
{"file": ".moluoxixi/spec/security/index.md", "reason": "Security guidelines"}
```
```
...
- **Files Affected**:
  - `.moluoxixi/scripts/task.py` (init-context function)
...
- Initialized project-local skill
- Base Moluoxixi version: 0.3.0
```
...
When working with multiple Moluoxixi projects:

```
~/projects/
├── project-a/
│   └── .claude/skills/project-local/   # Project A customizations
├── project-b/
│   └── .claude/skills/project-local/   # Project B customizations
└── project-c/
    └── .claude/skills/project-local/   # Project C customizations

~/.claude/skills/
└── meta/                        # Shared meta-skill (vanilla Moluoxixi)
```

**Each project has its own `project-local`** documenting that project's specific customizations.

**The meta-skill is shared** and documents vanilla Moluoxixi.

---

## Upgrade Workflow

When upgrading Moluoxixi to a new version:

### 1. Review New Version Changes

```bash
# Compare new meta-skill with current
diff -r ~/.claude/skills/meta/references/ \
        ./new-meta/references/
```

### 2. Check for Conflicts

Review each customization in `project-local`:
...
cp -r ~/.claude/skills/meta ~/.claude/skills/meta.backup

# Update meta-skill
cp -r ./new-meta/* ~/.claude/skills/meta/
```

### 4. Update Project Skills

Add migration note to `project-local`:

```markdown
### 2026-02-01 - Upgraded to Moluoxixi 0.4.0
...
When an AI modifies Moluoxixi, it MUST:

1. **Check** if `project-local` exists in the project
...
**Never** modify `meta` for project-specific changes.

**Always** tell the user what was documented.

Example AI response:
> "I've added the `/moluoxixi:deploy` command and documented it in `.claude/skills/project-local/SKILL.md` under the Commands section."
