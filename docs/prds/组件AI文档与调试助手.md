...
> 状态：**已定稿（FINALIZED 2026-06-13）**。所有关键决策已确认，业务 `MISSING` 已清零；仅保留 `MISSING verify script`（环境项，不阻断）。可作为下游架构设计的输入。
>
> 已确认决策（2026-06-13）：
...
> - 知识库检索 = **混合检索**，以**向量检索为主**（语义匹配组件契约/用法），叠加 **FTS 全文检索**等多路召回后融合。
...
> - 知识库 = 索引存于**包内本地文件**（构建产物 + `.cache/`，无外部 DB）；embedding 走 OpenAI 兼容端点 **`text-embedding-3-small`**；更新经**手动 CLI 命令 + 构建钩子**。
...
mindmap
  root((组件AI文档与调试助手))
    AST契约提取
      复用vue-docgen-api
      Props/Emits/Slots/Expose/Model
      类型与默认值
      门面出口扫描
    示例自动生成
      带类型提示用法
...
- 基于 AST 静态分析，自动提取本仓组件的完整契约（Props/Emits/Slots/Expose/Model/类型/默认值）。
...
- 本地知识库的构建（向量为主 + FTS 多路混合检索）与答案组装。
...
| 字段 | 含义 | 来源 | 规则 |
|---|---|---|---|
| 组件契约 | Props/Emits/Slots/Expose/Model 及类型/默认值 | `vue-docgen-api` 解析 SFC + 类型目录门面出口 | 以源码 AST 为准；与 `docs/out-components/` 冲突时标 `MISSING component docs drift` |
...
- 知识库索引状态机：`未构建` → `构建中` → `就绪` → `过期(源码变更)`。索引更新触发 = 手动 CLI 命令 + 构建钩子；索引存于包内本地文件（构建产物 + `.cache/`）。
