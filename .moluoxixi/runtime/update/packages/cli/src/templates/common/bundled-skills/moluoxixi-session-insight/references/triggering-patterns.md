...
Verbatim user phrasings that should make an AI reach for `moluoxixi mem`. Calibrate instinct against these — if a user message hits one of these patterns and you do not reach for `mem`, you probably missed an obvious recall.
...
Reach: `moluoxixi mem search "<symptom keyword>" --global --limit 10`, then `context` into the hit that looks closest.
...
Reach: `moluoxixi mem search "<decision keyword>"` to find the session, then `extract <id> --phase brainstorm` to recover the discussion.
...
Reach: `moluoxixi mem list --task <current-task-dir>` to find the most recent sessions tied to the active task, then `extract` the last one.
...
Reach: `moluoxixi mem search "<error message fragment>" --global`. Anchor on a short, distinctive token from the actual error string.
...
Reach: `moluoxixi mem search "<topic>" --global --limit 50` and scan the dates / projects in the listing. Optionally `extract` two or three for comparison.
