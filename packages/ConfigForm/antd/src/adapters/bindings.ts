...
import type { AntdConfigFormFieldBinding } from '../types'

/**
 * Ant Design Vue 字段组件的默认双向绑定协议。
 *
 * `AntdConfigForm` 本身不接入 runtime 插件系统，这里集中管理 UI adapter 映射，
...
/** 根据 Ant Design Vue 组件名读取字段默认绑定；未命中时使用 AntdConfigForm 的 value/update:value 默认值。 */
