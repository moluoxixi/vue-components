...
文档站使用独立包 `@moluoxixi/vitepress-theme-element-plus` 提供完整的 Element Plus 风格 VitePress 布局。导航、侧栏、移动端菜单、本页目录、暗色模式、文档翻页和 NotFound 都由主题包维护，不再继承 VitePress `DefaultTheme`。当前组件库只保留自己的站点配置、正文、示例和文档专属组件。
...
3. 维护目标组件库自己的组件目录和正文。API、Demo、Playground、GitHub 元数据等项目能力属于消费方，通过 Markdown 插件、全局组件和主题扩展点接入；主题包不声明或静默启用这些项目能力。
...
主题包是布局、Element Plus 安装、基础样式和通用交互的唯一来源；消费方负责站点身份、组件内容和项目数据。第一版固定复制自 Element Plus 文档主题的已记录 commit，后续修改直接在独立包中维护，不自动跟踪上游，也不在每个文档站复制一套主题源码。
