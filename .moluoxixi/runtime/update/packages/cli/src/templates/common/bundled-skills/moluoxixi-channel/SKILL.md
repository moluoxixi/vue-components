...
- `@moluoxixi/airules-moluoxixi-core` owns reusable channel/thread state, event append, seq allocation, context/title projection, reducers, and task helpers. The CLI owns flags, terminal rendering, prompts, worker lifecycle, and process exits.
...
- Long-term memory retrieval. Use durable forum channels for actionable issues, and `moluoxixi mem` (the `moluoxixi-session-insight` skill) for session/history search.
