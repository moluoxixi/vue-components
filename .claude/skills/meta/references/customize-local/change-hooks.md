...
Hooks are the automation layer that connects a platform to Moluoxixi. When the user wants to change "when context is injected," "how shell commands inherit a session," or "which files are read before an agent starts," hooks are usually the edit point.
...
| Hook | Purpose |
| --- | --- |
| session-start | Injects a Moluoxixi overview when a session starts, clears, or compacts. |
...
- Hooks should read project-local `.moluoxixi/`; they should not depend on Moluoxixi upstream source paths.
