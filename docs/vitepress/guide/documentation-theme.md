...
各平台使用独立命令：`sync-github-metadata`、`sync-gitlab-metadata`、`sync-gitee-metadata`、`sync-yunxiao-metadata` 和 `sync-local-metadata`；对应的 `validate-*-metadata` 命令可离线校验已提交快照。`validate-repository-metadata` 是 CI 的离线聚合入口，覆盖已经拥有真实快照的 GitHub、GitLab、Gitee、local 和 Yunxiao，不调用任何网络同步。GitLab 支持显式 web/API base URL、安装子路径和含 subgroup 的完整项目路径；私有项目通过运行时 `GITLAB_TOKEN` 使用 `PRIVATE-TOKEN` 或 Bearer 认证。自签名证书与代理由运行 Node 的受信任 CA/网络环境负责，采集器不会绕过 TLS 校验。Gitee 以公共云 REST v5 为基线，web/API base URL 可配置，但企业版必须按实例核验，使用可选 `GITEE_TOKEN`。云效同时表达中央站和地域租户 API；当前已用一个保留的私有 Codeup 验证仓库完成中央 API 的仓库、默认分支、组件提交和贡献者聚合验收，实时同步仍必须提供短期运行时 `YUNXIAO_TOKEN`。云效当前只公开提交历史和贡献者能力，Issues、源码、编辑、行号和贡献者资料链接保持关闭，直到有独立的真实路由证据。所有 collector 都不会把作者邮箱或 token 写入快照，分页也只能留在配置的 API origin 与路径范围内。
