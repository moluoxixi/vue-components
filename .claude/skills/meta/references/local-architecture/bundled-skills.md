...
| `session-insight` | Wraps the `node .moluoxixi/runtime/moluoxixi.mjs mem` CLI so an AI knows when and how to reach into past Claude Code / Codex / Pi Agent conversation logs. |
...
| `channel` | Capability skill teaching an AI when to reach for `node .moluoxixi/runtime/moluoxixi.mjs channel` for multi-agent collaboration, forum/thread persistent boards, and dispatcher-wait patterns. |
...
2. `node .moluoxixi/runtime/moluoxixi.mjs update` → `collectPlatformTemplates(platformId)` (in `configurators/index.ts`) → the same map, used to detect drift and to populate `.moluoxixi/.template-hashes.json`.
...
4. **No dispatch wiring is required.** `listDirectories("bundled-skills")` discovers the new directory automatically, so all platforms receive it on the next `moluoxixi init` or `node .moluoxixi/runtime/moluoxixi.mjs update`.
...
5. **Verify the distribution path** before shipping. Skipping any of these steps has historically caused features to be documented as bundled while the published npm tarball was missing the files:
   ...
   - `pnpm --filter @moluoxixi/airules-moluoxixi-cli build` copies the asset into `dist/templates/common/bundled-skills/<skill>/`.
   ...
   - `node .moluoxixi/runtime/moluoxixi.mjs update --dry-run` in that temp project reports "Already up to date!".

6. **Add a migration manifest entry** if the skill is added in a release that other projects will upgrade into. Without an explicit manifest entry the file will land via the standard "missing file" branch of `node .moluoxixi/runtime/moluoxixi.mjs update`, but a manifest makes the change visible in the changelog.
...
The supported pattern relies on the existing template-hash diff in `node .moluoxixi/runtime/moluoxixi.mjs update`:
...
3. The next `node .moluoxixi/runtime/moluoxixi.mjs update` detects the user modification and leaves the file untouched (Moluoxixi never overwrites user-modified files without an explicit `--force`).
...
- A future `node .moluoxixi/runtime/moluoxixi.mjs update --force` will overwrite local edits. Keep the override under version control so it can be reapplied if needed.
...
1. **Delete the directory in each platform skill root.** `node .moluoxixi/runtime/moluoxixi.mjs update` will see the file missing, compare against `.template-hashes.json`, and treat the deletion the same as any other user modification — it will not silently re-create the directory unless `--force` is passed.
...
- Do not couple bundled skills to a specific CLI binary (e.g. `node .moluoxixi/runtime/moluoxixi.mjs mem`) without surfacing the dependency in the skill's description and references — users on older releases may not have the command.
