...
adapter 不直接引入 `ant-design-vue`，只读取组件对象上的 `name`。字段已显式声明非默认绑定协议时不会被覆盖；默认 strict 模式下，组件名形如 Ant Design Vue 组件但没有映射时会直接抛错。Vue 函数组件必须先用 `asVueFunctionalComponent()` 包装再注册；若要复用内置绑定和只读映射，registry key 必须保持 `AInput` 等 canonical key。自定义组件名需要显式提供同 key 的 `bindings` 和 `readonlyAdapters`。
