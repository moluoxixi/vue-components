...
function createElementStubs(validateResult = true) {
  ...
  const validateField = vi.fn(async (
    _props?: unknown,
...
describe('element config form', () => {
  ...
  it('完整透传 Element Plus validate 和 validateField 参数', async () => {
    const { methods, stubs } = createElementStubs()
    const wrapper = mount(ElementConfigForm, {
      props: {
        fields: [],
        modelValue: { name: '表单', status: 'draft' },
      },
      global: { stubs },
    })
    const api = wrapper.vm as unknown as ElementConfigFormExpose<UserForm>
    const validateCallback = vi.fn()
    const validateFieldCallback = vi.fn()

    await api.validate(validateCallback)
    await api.validateField('name', validateFieldCallback)

    expect(methods.validate).toHaveBeenCalledWith(validateCallback)
    expect(validateCallback).toHaveBeenCalledWith(true, undefined)
    expect(methods.validateField).toHaveBeenCalledWith('name', validateFieldCallback)
    expect(validateFieldCallback).toHaveBeenCalledWith(true)
  })

  it('字符串容器组件可以接收具名配置 slots 和真实 slot props', () => {
    const { stubs } = createElementStubs()
    const NamedContainerStub = defineComponent({
      name: 'NamedContainerStub',
      setup(_props, { slots }) {
        return () => h('article', [
          slots.header?.({ source: 'header' }),
          slots.default?.({ source: 'default' }),
        ])
      },
    })
    const fields = [defineField<UserForm>({
      component: 'NamedContainerStub',
      slots: {
        default: ({ slotProps }) => h('span', { 'data-testid': 'named-default' }, String(slotProps.source)),
        header: ({ slotProps }) => h('span', { 'data-testid': 'named-header' }, String(slotProps.source)),
      },
    })]
    ...
    const wrapper = mount(ElementConfigForm, {
      ...
      global: {
        components: { NamedContainerStub },
        stubs,
      },
    })

    expect(wrapper.get('[data-testid="named-header"]').text()).toBe('header')
    expect(wrapper.get('[data-testid="named-default"]').text()).toBe('default')
  })
