    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python \"$CLAUDE_PROJECT_DIR/.claude/hooks/inject-subagent-context.py\"",
            "timeout": 30
          }
        ]
      }
    ...
    "SubagentStop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python \"$CLAUDE_PROJECT_DIR/.claude/hooks/ralph-loop.py\"",
            "timeout": 300
          }
        ]
      }
...
**Injects:**
- Developer identity from `.moluoxixi/.developer`
...
TOOL_INPUT='{"subagent_type":"implement","prompt":"test"}' \
  python .claude/hooks/inject-subagent-context.py
```
