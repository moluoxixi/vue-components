...
function parseManifest(input: unknown, providerId: string): ProjectTemplateManifest | TemplateCatalogDiagnostic {
  ...
  if (version !== PROJECT_TEMPLATE_VERSION) {
    return { code: 'TEMPLATE_VERSION_INVALID', message: `Template "${id}" requires version ${PROJECT_TEMPLATE_VERSION}.`, providerId, templateId: id, path: 'manifest.version' }
  }
...
export function analyzeTemplateEligibility(
