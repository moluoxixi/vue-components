...
import type { GithubMetadataExpectation, GithubMetadataSnapshot } from '../../repository'
import { mkdtempSync, readdirSync, readFileSync, rmSync, writeFileSync } from 'node:fs'
import { tmpdir } from 'node:os'
import { join } from 'node:path'
import { afterEach, describe, expect, it } from 'vitest'
import { assertGithubMetadataSnapshot } from '../../repository'
import {
...
function createGithubFetch(
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (url.includes('/commits?')) {
      ...
      expect(url).toContain('path=packages%2Ffixture%2FCopyText')
      return githubResponse([{
        sha: '1'.repeat(40),
...
describe('gitHub documentation metadata', () => {
  ...
  it('paginates component data, pins the configured branch, and applies contributor policy', async () => {
    ...
    const metadata = await createGithubMetadata({
      ...
      defaultBranch: 'main',
      components: [{ name: 'CopyText', path: fixtureComponentPathByName.CopyText! }],
