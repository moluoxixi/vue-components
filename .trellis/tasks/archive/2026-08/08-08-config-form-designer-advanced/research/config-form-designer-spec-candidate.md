...
interface DesignerNodeLocation {
  node: DesignerNode
  parent?: DesignerNode
}

function findDesignerNode(
...
- `node.span` belongs only to direct children of the root ConfigForm grid. Card and Section children use natural container flow, Flex uses item width, and Grid uses its local column contract. Nested nodes do not expose a Span setter, while imported nested span values remain parseable and export unchanged.
...
- Selection chrome uses exactly one dashed frame, positioned 5px outside the selected node cell without participating in layout. The action bar attaches to the frame's top-right edge, and `:focus-within` hides both when focus leaves the node while preserving property context.
- Container previews preserve their adapter-native structure. Non-empty Card, Tabs, Collapse, Flex, and Grid slots add no generic designer border, background, padding, label, or instruction text; Section alone uses a lightweight heading-and-divider hierarchy.
- An empty container slot renders one icon-only insertion surface. Its resting border is transparent, its center affordance is an unframed plus icon over a quiet insertion line, and the accent dashed boundary appears only while a drag is active.
- Flex and Grid may add low-contrast structural guides to the same empty insertion surface. Guides must not create another container frame, must become more visible only during drag interaction, and must disappear with the empty surface once content exists.
...
| Root spans are `24 / 8 / 8 / 8` | First node occupies one row; the three 8-cell nodes share the following row | None |
| Imported nested node has `span` | Preserve it during parse/export, but do not apply or offer it in the property panel | None; container layout owns sizing |
...
| Focus leaves a selected node for the property panel | Hide the frame/action bar but preserve controller selection | No property-context loss |
| Container slot is empty or receives a drag | Show one icon-only quiet insertion surface; strengthen its dashed accent and layout guides only during drag | No generic nested frame, canvas instructions, or duplicate selection chrome |
...
// Good: root grid ownership is explicit.
const rootNodes = [
  { id: 'full', span: 24 },
  { id: 'left', span: 8 },
  { id: 'middle', span: 8 },
...
// Bad: presenting a root-grid setter for a nested container child.
const nestedSetters = [...commonSetters, spanSetter]
```
...
- Root span tests: Runtime and designer independently assert `24 / 8 / 8 / 8`; browser E2E checks row coordinates in Runtime preview before comparing the designer canvas.
- Nested span tests: imported values round-trip unchanged, no grid style consumes them, and the property panel exposes no Span setter.
...
- Selection-frame tests: assert one full-cell dashed frame, top-right action alignment, focus-loss hiding, and stable node geometry before issuing drag coordinates.
- Container-canvas tests: cover Section/Card/Tabs/Collapse/Flex/Grid for both adapters, assert native non-empty flow, one icon-only empty surface, transparent resting chrome, drag-state accent reset, Flex/Grid guides, and desktop/390px viewport bounds; retain browser screenshots as manual visual evidence.
...
// Nested nodes misleadingly expose a value their container ignores.
const setters = [...fieldSetters, spanSetter]
```
...
const location = findDesignerNode(document, selectedNode.id)
const setters = location?.parent ? fieldSetters : [...fieldSetters, spanSetter]
```
