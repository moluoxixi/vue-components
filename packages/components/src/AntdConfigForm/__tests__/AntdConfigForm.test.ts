...
describe('antd config form', () => {
  ...
  it('字符串容器组件可以接收具名配置 slots 和真实 slot props', () => {
    ...
    const fields = [defineField<UserForm>({
      component: 'AntdNamedContainerStub',
      slots: {
        default: ({ slotProps }) => h('span', { 'data-testid': 'antd-named-default' }, String(slotProps.source)),
        header: ({ slotProps }) => h('span', { 'data-testid': 'antd-named-header' }, String(slotProps.source)),
      },
    })]
    ...
    const wrapper = mount(AntdConfigForm, {
      ...
      global: {
        components: { AntdNamedContainerStub: NamedContainerStub },
        stubs,
      },
    })

    expect(wrapper.get('[data-testid="antd-named-header"]').text()).toBe('header')
    expect(wrapper.get('[data-testid="antd-named-default"]').text()).toBe('default')
  })
