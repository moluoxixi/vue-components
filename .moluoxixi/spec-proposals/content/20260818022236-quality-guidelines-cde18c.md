...
- Local history is collected with component-scoped `git log --use-mailmap` queries.
...
- The pre-commit snapshot records history through the current `HEAD`. It cannot contain the commit being created because that commit object and SHA do not exist while the hook runs; the next refresh incorporates it.
