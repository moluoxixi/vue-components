...
interface FieldValidationRequest {
  ...
  /** 当前快照是否只包含可由顶层 data-property 日志安全重放的字段。 */
...
interface FieldValidationState {
  ...
  running: boolean
  /** 当前字段按调用顺序等待执行的请求；只有兼容的交互请求会合并。 */
  pending: FieldValidationRequest[]
  /** pending 中首个未消费请求的下标，避免 shift 反复搬移长队列。 */
  pendingStartIndex: number
  /** 当前正在执行的请求；dispose 时用于显式结束调用方 Promise。 */
...
export function useFormValidation(options: UseFormValidationOptions): UseFormValidationResult {
  ...
  function syncValueChangeRetention(): void {
    // 请求版本只会随 valuesRevision 单调增加，因此 Map 的首个 key 始终是最旧版本。
