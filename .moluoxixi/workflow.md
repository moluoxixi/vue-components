...
```bash
python ./.moluoxixi/scripts/init_developer.py <your-name>
```
...
```bash
python ./.moluoxixi/scripts/get_context.py --mode packages   # list packages / layers
```
...
```bash
# Task lifecycle
python ./.moluoxixi/scripts/task.py create "<title>" [--slug <name>] [--complexity lightweight|complex] [--parent <dir>]
python ./.moluoxixi/scripts/task.py set-complexity <name> lightweight|complex [--signal <name>] [--reason <text>]
python ./.moluoxixi/scripts/task.py set-execution-mode <name> manual|auto [--user-authorized]
python ./.moluoxixi/scripts/task.py start <name> --user-approved  # manual mode after final plan review
python ./.moluoxixi/scripts/task.py current --source      # show active task and source
python ./.moluoxixi/scripts/task.py finish                # clear active task (triggers after_finish hooks)
python ./.moluoxixi/scripts/task.py archive <name>        # move to archive/{year-month}/
python ./.moluoxixi/scripts/task.py list [--mine] [--status <s>]
python ./.moluoxixi/scripts/task.py list-archive

# Code-spec context (injected into implement/check agents via JSONL).
# `implement.jsonl` / `check.jsonl` are seeded on `task create` for sub-agent-capable
# platforms; the AI curates real spec + research entries during planning when needed.
python ./.moluoxixi/scripts/task.py add-context <name> <action> <file> <reason>
python ./.moluoxixi/scripts/task.py list-context <name> [action]
python ./.moluoxixi/scripts/task.py validate <name>

# Task metadata
python ./.moluoxixi/scripts/task.py set-branch <name> <branch>
python ./.moluoxixi/scripts/task.py set-base-branch <name> <branch>    # PR target
python ./.moluoxixi/scripts/task.py set-scope <name> <scope>

# Hierarchy (parent/child)
python ./.moluoxixi/scripts/task.py add-subtask <parent> <child>
python ./.moluoxixi/scripts/task.py remove-subtask <parent> <child>

# PR creation
python ./.moluoxixi/scripts/task.py create-pr [name] [--dry-run]
```

> Run `python ./.moluoxixi/scripts/task.py --help` to see the authoritative, up-to-date list.
...
```bash
python ./.moluoxixi/scripts/add_session.py --title "Title" --commit "hash" --summary "Summary"
```

### Context Script

```bash
python ./.moluoxixi/scripts/get_context.py                            # full session runtime
python ./.moluoxixi/scripts/get_context.py --mode packages            # available packages + spec layers
python ./.moluoxixi/scripts/get_context.py --mode phase --step <X.Y>  # detailed guide for a workflow step
```
...
<!--
  ...
  Editing checklist:
    ...
    - Run `node .moluoxixi/runtime/moluoxixi.mjs update` after editing to push the new bodies to
...
Load `brainstorm`; stay in planning.
...
<!-- Per-turn breadcrumb: shown throughout Phase 1 when codex.dispatch_mode=inline.
     ...
     the inline workflow loads `before-dev` instead of injecting JSONL
     into a sub-agent. -->
...
Persist complexity with `task.py set-complexity`. Lightweight: `prd.md` can be enough. Complex: finish `prd.md`, `design.md`, and `implement.md`; ask for review before `task.py start --user-approved`.
Multi-deliverable scope: consider a parent task plus independently verifiable child tasks; dependencies must be written in child artifacts, not implied by tree position.
Inline mode: skip jsonl curation; Phase 2 reads artifacts/specs via `before-dev`.
[/workflow-state:planning-inline]
...
Flow: `before-dev` -> edit -> `check` -> validation -> `update-spec` -> commit (Phase 3.4) -> `/moluoxixi:finish-work`.
...
[Claude Code, Cursor, OpenCode, codex-sub-agent, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi, Oh My Pi, ZCode, Reasonix, Trae, DeepSeek Harness, Grok, Kimi Code, Snow]
...
- Planning or unclear requirements -> `brainstorm`.
- Before editing -> `before-dev`; after editing -> `check`.
- Repeated debugging -> `break-loop`; spec updates -> `update-spec`.
...
```bash
python ./.moluoxixi/scripts/get_context.py --mode phase --step <step>
# e.g. python ./.moluoxixi/scripts/get_context.py --mode phase --step 1.1
```
...
```bash
python ./.moluoxixi/scripts/task.py create "<task title>" --slug <name> --complexity lightweight|complex
```
...
Skip when `python ./.moluoxixi/scripts/task.py current --source` already points to a task.

#### 1.1 Requirement exploration `[required · repeatable]`

Load the `brainstorm` skill and explore requirements interactively with the user per the skill's guidance.
...
```bash
python ./.moluoxixi/scripts/task.py add-context "$TASK_DIR" implement "<path>" "<reason>"
python ./.moluoxixi/scripts/task.py add-context "$TASK_DIR" check "<path>" "<reason>"
```
...
For complex sub-agent-dispatch tasks, the planning gate is strict. Ready gate: both `implement.jsonl` and `check.jsonl` must contain at least one real `{"file": "...", "reason": "..."}` entry before `task.py start`. The seed `_example` row alone is not ready. Lightweight tasks stay lightweight and do not need curated JSONL to pass the hard gate.
...
Skip this step. Context is loaded directly by the `before-dev` skill in Phase 2.
...
```bash
python ./.moluoxixi/scripts/task.py start <task-dir> --user-approved
```
...
```bash
python ./.moluoxixi/scripts/task.py set-execution-mode <task-dir> auto --user-authorized --reason "<user request>"
python ./.moluoxixi/scripts/task.py start <task-dir>
```
...
- For Codex, `SubagentStart` supplies native context injection. Its generated agent definitions also retain a child-side pull fallback for sessions where project hooks are unavailable or not approved.
...
1. Load the `before-dev` skill to read project guidelines
...
[Claude Code, Cursor, OpenCode, codex-sub-agent, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi, Oh My Pi, ZCode, Reasonix, Trae, Grok, Kimi Code, Snow]
...
Load the `check` skill and verify the code per its guidance:
- Spec compliance
...
**Final pass (before Phase 3.4 commit)**: the last 2.2 of a task must run full-scope, not just on the latest implement chunk. List all affected packages with `python ./.moluoxixi/scripts/get_context.py --mode packages`, then load each package's spec index Quality Check section. This catches cross-layer / multi-package issues a mid-iteration local 2.2 cannot.
...
If this task involved repeated debugging (the same issue was fixed multiple times), load the `break-loop` skill to:
...
Load the `update-spec` skill and review whether this task produced new knowledge worth recording:
...
Directly edit the body of the corresponding `[workflow-state:STATUS]` block. After editing, run `node .moluoxixi/runtime/moluoxixi.mjs update` (if you're a template maintainer) or restart your AI session (if you're customizing your own project) — no script changes required.
