...
- R14: Make repository quality gates truthful for the documentation work: root lint must not be dominated by managed Trellis/Codex output, and type checking must cover the docs package plus package-level test suites that are currently omitted by incorrect or incomplete tsconfig includes.
