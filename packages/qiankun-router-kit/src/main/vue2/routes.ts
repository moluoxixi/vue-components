// 应用清单 → vue-router 3 的路由记录。三个框架各有一份这样的翻译，
...
import type { Vue2Component, Vue2Route, Vue2RouteConfig } from './types'

/**
 * 应用清单 → 路由记录。每个 activeRule 前缀生成一条 `${prefix}/:pathMatch*`：
 * 前缀由主应用路由承接，前缀之后的部分交给子应用自己的 router。
 *
 * 通配写法与 vue3 层不同：vue-router 3 用 path-to-regexp 1.x，`/:pathMatch*` 是
 * “零或多段”，所以 `/sub-a` 与 `/sub-a/about` 都能匹配到同一条记录。写成
 * `${prefix}/*` 反而匹配不上 `/sub-a` 本身（深链接直达前缀会掉进 404）。
 *
