...
属性面板是设计器自身的 chrome，不跟随画布 Provider 切换 UI 库：`DesignerPropertyForm` 将 setter 声明编译为 `ConfigFormField[]` 后交给注入的 Renderer，简单控件（`text/textarea/number/boolean/select`）由 Designer 核心的 `DEFAULT_DESIGNER_PROPERTY_CONTROLS` 直接提供固定的 Element Plus 控件，`layers` 注册的同名控件可按需覆盖。适配器侧只剩可选的 `defaultValue` 槽位：Designer 核心只传递 `modelValue`、`kind`、`options`、`disabled` 并监听 `update:modelValue`，Element Plus adapter 注册 `ElementDefaultValueSetter`（含日期/时间选择），Workbench 为 antd 物料复用同一 EP 默认值控件；未注册时继续使用核心原生 fallback。Provider 组件及其样式不会倒灌到 UI 库无关的 Designer 核心。
