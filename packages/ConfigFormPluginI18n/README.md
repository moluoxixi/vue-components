...
ConfigForm 的官方 i18n 字段转换插件。本包不再提供 token API，也不会修改字段对象；它只在 runtime 的 `transformField(field)` 阶段，根据 `fields` 映射为对应字段补充 `label`、`props` 和 `slots` 文案。
...
- `slots`：递归解析 slots 中的文案描述，适合简单文本插槽。
...
createI18nPlugin({
  ...
  fields: {
    role: {
      ...
      slots: {
        help: { key: 'field.role.help', params: { min: 1 } },
      },
    },
  },
})
...
字段原始配置始终优先于插件补充值，因此用户在字段上显式声明的 `label`、`props.placeholder` 等不会被 i18n 插件覆盖。整体优先级保持 `用户 > 插件 > 内置默认值`。
