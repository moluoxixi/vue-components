...
Complete guide to `.moluoxixi/worktree.yaml` configuration.
...
**Note**: Moluoxixi uses a custom YAML parser (not PyYAML). Supports basic key-value pairs and arrays; complex nested structures may not work.
...
Moluoxixi uses a custom YAML parser (not PyYAML) with these limitations:
...
cat .moluoxixi/.ralph-state.json
```
