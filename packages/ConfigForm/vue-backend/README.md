...
ConfigForm Canonical IR 的 Vue 运行时后端。它把 Compiler 产出的不可变 Surface 编译结果绑定到 Vue 组件、校验器和只读渲染协议，生成 `ConfigFormRenderer` 可直接消费的渲染配置。

本包不读取或修复 `ProjectDocument`，也不负责 UI 库物料注册；调用方必须传入同一快照生成的 `SurfaceCompilation`/`ProjectCompilation` 和与 registry identity 匹配的 binding resolver。
...
```ts
import type { CompileCanonicalSurfaceRuntimeInput, VueRuntimeBindingResolver } from '@moluoxixi/config-form-vue-backend'
import { compileCanonicalSurfaceRuntime } from '@moluoxixi/config-form-vue-backend'

export function compileVueRuntime(input: CompileCanonicalSurfaceRuntimeInput, resolver: VueRuntimeBindingResolver) {
  const result = compileCanonicalSurfaceRuntime(input, resolver)
  if (!result.success) {
...
- `VueRuntimeCompileResult`、`VueSurfaceRuntimeArtifact`、`VueRuntimeRendererConfig`：编译结果与渲染配置类型。
