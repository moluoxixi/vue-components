...
- Selection chrome belongs to the real node layout cell: one dashed pseudo-element uses `inset: -5px`, and one absolutely positioned action bar attaches to its top-right edge. Both are gated by `:focus-within`, require no component measurement, and disappear on focus loss without clearing the selected node's property context.
...
| Focus moves from a selected node to the property panel | Hide the frame and action bar while retaining controller selection | No document mutation or property-context loss |
...
- Selection chrome tests: assert one full-cell frame, no legacy overlay/footprint, unchanged node geometry, toolbar alignment to the frame, and focus-loss hiding with retained selection.
