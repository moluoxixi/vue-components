...
import { transformDefineFieldSource } from '../source-inject'
import { createOpenInEditorMiddleware } from './open-in-editor-middleware'
...
function resolvePackageEntry(metaUrl: string, baseName: string): string {
  ...
  const sourceEntry = normalizePath(modulePath).endsWith('/src/services/plugin.ts')
  const moduleDir = sourceEntry ? resolve(dirname(modulePath), '..') : dirname(modulePath)
  return normalizePath(sourceEntry
    ? resolve(moduleDir, baseName, 'index.ts')
    : resolve(moduleDir, `${baseName}.js`))
}
