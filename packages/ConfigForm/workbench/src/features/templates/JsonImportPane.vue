...
function migrationMessage(item: PreparedConfigImport['migrations'][number]): string {
  return locale.value.t(`import.migration.${item.code}`, item.message)
}
    ...
    <div class="json-import-layout">
      ...
      <section class="json-import-inspection" :class="{ 'is-mobile-hidden': mobileStage === 'source' }">
        <div class="json-import-diagnostics" :class="{ 'is-mobile-hidden': mobileStage !== 'diagnostics' }">
          <header>
            <div><SearchCheck :size="17" aria-hidden="true" /><h2>{{ locale.t('import.diagnostics', 'Diagnostics') }}</h2></div>
          ...
          <div v-else-if="prepared" class="json-import-report">
            <dl>
              ...
              <div v-if="prepared.summary.schemaVersion"><dt>{{ locale.t('import.schemaVersion', 'Project schema') }}</dt><dd>v{{ prepared.summary.schemaVersion }}</dd></div>
            </dl>
            <section class="json-import-migrations">
              <h3>{{ locale.t('import.migrations', 'Migrations') }}</h3>
              <p v-if="prepared.migrations.length === 0">{{ locale.t('import.noMigrations', 'No migration required') }}</p>
              <ol v-else>
                <li v-for="migration in prepared.migrations" :key="`${migration.code}:${migration.path}`">
                  <code>{{ migration.code }}</code>
                  <strong>{{ migration.fromVersion }} → {{ migration.toVersion }}</strong>
                  <small>{{ locale.t('import.path', 'Scope') }}: {{ migration.path }}</small>
                  <span>{{ migrationMessage(migration) }}</span>
                </li>
              </ol>
            </section>
          </div>
        ...
        <div class="json-import-preview" :class="{ 'is-mobile-hidden': mobileStage !== 'preview' }">
          <header><div><CheckCircle2 :size="17" aria-hidden="true" /><h2>{{ locale.t('import.preview', 'Preview') }}</h2></div></header>
