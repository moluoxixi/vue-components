...
export function compileDataFixture() {
  ...
  const document: ProjectDocument = {
    version: 4, id: 'data-preview', name: 'Data preview', homePageId: 'home', pageOrder: ['home'],
    registryLock: registry.lock, settings: {}, resources: {},
    pagesById: { home: {
      ...
      runtime: {
        ...
        dataSources: [{ id: 'choices', name: 'Choices', auto: true, cacheTtlMs: 60_000,
          request: { url: '/choices', query: { region: { $ref: { kind: 'variable', variableId: 'region' } } } } }],
      },
      graph: { version: 2, props: {}, form: {},
        root: ['choice', 'result'].map(nodeId => ({ nodeId, placement: {} })),
        nodesById: {
  ...
  const exportSource = () => createCanonicalProjectSourceExport(project.compilation, {
    ...
    resolveBinding: () => ({ component: 'test.choice', contractVersion: identity.contractVersion, contractFingerprint: identity.fingerprint,
      configComponent: 'choice', tag: 'DataControl', render: 'component', valueProp: 'modelValue', trigger: 'update:modelValue' }),
  })
