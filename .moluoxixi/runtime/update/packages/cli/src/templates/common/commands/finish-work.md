...
```bash
{{PYTHON_CMD}} ./.moluoxixi/scripts/get_context.py --mode record
```
...
```bash
{{PYTHON_CMD}} ./.moluoxixi/scripts/task.py archive <task-name>
```
...
```bash
{{PYTHON_CMD}} ./.moluoxixi/scripts/add_session.py \
  --title "Session Title" \
  --commit "hash1,hash2" \
