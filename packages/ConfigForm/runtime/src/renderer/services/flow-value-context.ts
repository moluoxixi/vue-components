...
/** Resolve stable field IDs against the supplied run-local values. */
export function createRendererFlowValueContext(
...
export function selectRendererFieldInstance(
  ...
  eventScope: ConfigFormScopePath,
  ...
  if (!context) {
    const selected = selectConfigFormScopePath(eventScope, selector)
    return instances.find(instance => sameRendererScope(instance.address.scope, selected))
  }

  const scopes = new Map(context.schema.valueScopes.map(scope => [scope.nodeId, scope]))
  ...
  const current = context.sourceNodeId !== undefined && scopes.has(context.sourceNodeId)
    ...
    : sourceField?.scopeId
  // An unknown component owner cannot be inferred from a row chain containing only arrays.
  ...
  if (!arrayChain.every((scopeId, index) => eventScope[index]?.scopeId === scopeId))
    return undefined
  const selected = eventScope.slice(0, arrayChain.length)
