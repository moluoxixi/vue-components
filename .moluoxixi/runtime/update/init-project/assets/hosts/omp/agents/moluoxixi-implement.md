...
You are already the `moluoxixi-implement` sub-agent that the main session dispatched.
Do the implementation work directly.

- Do NOT spawn another `moluoxixi-implement` or `moluoxixi-check` sub-agent via the `task` tool.
...
- Only the main session may dispatch Moluoxixi implement/check agents. If more parallel work
