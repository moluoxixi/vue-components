...
Moluoxixi workflow skills usually share one semantic set: brainstorm, before-dev, check, update-spec, break-loop. Multi-file built-in skills such as `moluoxixi-meta` use layered references.
...
| Kimi Code | `.agents/skills/`, `.kimi-code/skills/` (commands delivered as `/skill:moluoxixi-*` skills) |
...
1. **Keep entry files short; references carry long content**. This matters especially for multi-file skills like `moluoxixi-meta`.
...
4. **Put project-specific capabilities in local skills**. Do not put team-private flows into public `moluoxixi-meta`.
