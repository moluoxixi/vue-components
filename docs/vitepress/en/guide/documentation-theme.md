...
Repository metadata has two strictly independent committed snapshots: `.vitepress/github-metadata.json` and `.vitepress/git-local-metadata.json`. A site must explicitly select `github` or `git-local` through `docsSite.metadataSource`; there is no `auto` mode, file fallback, or cross-source merge. Normal `dev` and `build` commands validate and consume only the selected snapshot without network access. `pnpm --filter @moluoxixi/docs sync-github-metadata` refreshes issues, profiles, and commits through the GitHub API. `pnpm --filter @moluoxixi/docs sync-git-local-metadata` refreshes commits and contributors from the configured default branch in complete local Git history, rejects shallow clones, and never writes author email to the snapshot.

The existing Husky `pre-commit` hook refreshes the local Git snapshot and stages only `.vitepress/git-local-metadata.json`; it does not stage other workspace files. The scanner resolves the configured default branch rather than an arbitrary checked-out `HEAD`. A pending commit SHA does not exist until after the hook completes, so a commit made on the default branch covers history through that branch's pre-commit head; the newly created commit appears on the next refresh. Run `validate-github-metadata`, `validate-git-local-metadata`, or the configuration-aware `validate-selected-metadata` for explicit validation.
