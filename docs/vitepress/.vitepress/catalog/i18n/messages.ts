...
const messages: Record<DocsLocale, DocsMessages> = {
  'zh-CN': {
    ...
    changelog: { aria: '{name} 更新日志', empty: '暂无提交记录', commitLink: '查看提交 {sha}' },
  ...
  'en-US': {
    ...
    changelog: { aria: '{name} changelog', empty: 'No commits yet', commitLink: 'View commit {sha}' },
...
export function getLocalizedUtilityGroups(locale: DocsLocale): LocalizedDocUtilityGroup[] {
  const localizedGroups = locale === 'en-US' ? englishUtilityGroups : undefined
  const localizedUtilities = locale === 'en-US' ? englishUtilities : undefined
  return utilityGroups
    .map(group => ({
      description: localizedGroups?.[group.id].description ?? group.description,
      id: group.id,
      items: group.items
        .flatMap((utility): LocalizedDocUtility[] => {
          const sourcePath = utility.sourcePaths[locale]
          if (!sourcePath)
            return []
          const { sourcePaths: _sourcePaths, ...baseUtility } = utility
          return [{
            ...baseUtility,
            ...localizedUtilities?.[utility.packageName],
            sourcePath,
          }]
        }),
      title: localizedGroups?.[group.id].title ?? group.title,
    }))
    .filter(group => group.items.length > 0)
}

export function getLocalizedUtilities(locale: DocsLocale): LocalizedDocUtility[] {
