import type { ReadonlyAdapter } from '..'
import type { FormNodeConfig } from '@/types'
import { describe, expect, it } from 'vitest'
import { createFormRuntime, resolveReadonlyAdapter } from '..'

describe('readonly component registry', () => {
  it('preserves a registered component key for readonly adapter lookup', () => {
    ...
    const runtime = createFormRuntime({
      components: { RegisteredInput: AnonymousInput },
      readonlyAdapters: { RegisteredInput: adapter },
    })
    const node = runtime.transformField({
      component: 'RegisteredInput',
      field: 'name',
    })
    if (!('field' in node))
      throw new Error('Expected a resolved bound node')

    expect(node.resolvedComponentKey).toBe('RegisteredInput')
  ...
  it('supports an explicitly registered empty-string key', () => {
    const adapter: ReadonlyAdapter = ({ value }) => String(value)
    const runtime = createFormRuntime({
      components: { '': {} },
