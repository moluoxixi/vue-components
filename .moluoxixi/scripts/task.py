...
Examples:
  ...
  python task.py create "Child task" --slug child --parent .moluoxixi/tasks/01-21-parent
  python task.py add-context <dir> implement .moluoxixi/spec/cli/backend/auth.md "Auth guidelines"
  python task.py set-branch <dir> task/add-login
  python task.py start .moluoxixi/tasks/01-21-add-login
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print(
            "  python ./.moluoxixi/scripts/get_context.py --mode phase --step 1",
            file=sys.stderr,
        )
        print(
