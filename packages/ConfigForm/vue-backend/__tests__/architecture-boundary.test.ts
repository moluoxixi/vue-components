...
describe('vue backend architecture boundary', () => {
  it('keeps one current package entry and indexed responsibility directories', () => {
    ...
    expect(source).not.toMatch(/compileCanonicalPage|PageCompilation|CanonicalPageIdentity|pagesById|pageId/)
    expect(source).not.toMatch(/optionSource|conditions|reactions|page\.runtime/)
    expect(source).toContain('compileCanonicalSurfaceRuntime')
    expect(source).toContain('VueSurfaceRuntimeArtifact')
  })
})
