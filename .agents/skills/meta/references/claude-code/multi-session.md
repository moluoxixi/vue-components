...
│                                                                             │
│  /moluoxixi:parallel → Configure task → start.py                             │
│                                           │                                 │
│                                           │ Creates worktree               │
│                                           │ Starts agent                   │
│                                           ▼                                 │
...
Location: `.moluoxixi/worktree.yaml`
...
copy:
  - .moluoxixi/.developer      # Developer identity
...
```bash
python .moluoxixi/scripts/multi_agent/start.py <task-dir>
```
...
2. Create git worktree:
   ```bash
   git worktree add -b <branch> ../moluoxixi-worktrees/<branch>
   ```
...
**Example**:
```bash
python .moluoxixi/scripts/multi_agent/start.py .moluoxixi/tasks/01-31-add-login-taosu
# Output: Started agent in ../moluoxixi-worktrees/feature/add-login
```
...
```bash
# Overview
python .moluoxixi/scripts/multi_agent/status.py

# Detailed view
python .moluoxixi/scripts/multi_agent/status.py --detail <task-name>

# Watch mode
python .moluoxixi/scripts/multi_agent/status.py --watch <task-name>

# View logs
python .moluoxixi/scripts/multi_agent/status.py --log <task-name>

# Show registry
python .moluoxixi/scripts/multi_agent/status.py --registry
```
...
Resume stopped sessions:
  cd ../moluoxixi-worktrees/feature/fix-api && claude --resume <session-id>
```
...
```bash
python .moluoxixi/scripts/multi_agent/create_pr.py [--dry-run]
```

**Actions**:
1. Stage changes: `git add -A`
2. Exclude: `git reset .moluoxixi/workspace/`
...
```bash
# Specific worktree
python .moluoxixi/scripts/multi_agent/cleanup.py <branch-name>

# All merged worktrees
python .moluoxixi/scripts/multi_agent/cleanup.py --merged

# All worktrees (with confirmation)
python .moluoxixi/scripts/multi_agent/cleanup.py --all
```

**Actions**:
1. Archive task to `.moluoxixi/tasks/archive/YYYY-MM/`
...
```bash
python .moluoxixi/scripts/multi_agent/plan.py \
  --name <task-slug> \
...
**Location**: `.moluoxixi/workspace/<developer>/.agents/registry.json`
  ...
  "agents": [
    {
      ...
      "worktree_path": "/abs/path/to/moluoxixi-worktrees/feature/add-login",
      "pid": 12345,
      "started_at": "2026-01-31T10:30:00",
      "task_dir": ".moluoxixi/tasks/01-31-add-login-taosu"
    }
  ]
}
```
...
```bash
# Create task
python .moluoxixi/scripts/task.py create "Add login" --slug add-login

# Configure
python .moluoxixi/scripts/task.py init-context <task-dir> fullstack
python .moluoxixi/scripts/task.py set-branch <task-dir> feature/add-login
...
```bash
python .moluoxixi/scripts/multi_agent/status.py --watch add-login
```
...
# Cleanup
python .moluoxixi/scripts/multi_agent/cleanup.py feature/add-login
```
...
```bash
# Session 1
python .moluoxixi/scripts/multi_agent/start.py .moluoxixi/tasks/01-31-add-login

# Session 2 (immediately)
python .moluoxixi/scripts/multi_agent/start.py .moluoxixi/tasks/01-31-fix-api

# Session 3
python .moluoxixi/scripts/multi_agent/start.py .moluoxixi/tasks/01-31-update-docs
...
# Resume
cd ../moluoxixi-worktrees/feature/task-name
...
**State** (`.moluoxixi/.ralph-state.json`):
```json
{
...
4. Check `.moluoxixi/.ralph-state.json`
...
# Manual edit
vim .moluoxixi/workspace/<dev>/.agents/registry.json
```
