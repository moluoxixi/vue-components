...
const ElementDatasetList = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const total = computed(() => Number.isInteger(props.itemsTotal) && (props.itemsTotal ?? -1) >= 0
      ? props.itemsTotal!
      : props.items.length)
    return () => h('section', { class: 'mx-element-dataset-list' }, [
      h('div', { class: 'mx-element-dataset-list__items', role: 'list' }, props.items.map((item, index) => {
        const key = itemKey(item, index)
        const selected = selectedKey.value === key
        return h('button', {
          'key': key,
          'aria-pressed': selected,
          'class': ['mx-element-dataset-list__item', selected && 'is-selected'],
          'type': 'button',
          'onClick': () => {
      ...
      h('small', { class: 'mx-element-dataset-list__summary', role: 'status' }, `${props.items.length} / ${total.value}`),
    ])
  },
})
...
export default defineConfigFormComponentMaterial<Component>({
  name: 'list',
  order: 150,
  value: { component: ElementDatasetList },
})
