...
function randomIdentity(kind: TemplateIdentityKind, source: string): string {
  const nonce = typeof globalThis.crypto?.randomUUID === 'function'
    ? globalThis.crypto.randomUUID()
    : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`
  return `${source.trim() || kind}-${nonce}`
}

export const DEFAULT_TEMPLATE_IDENTITY_FACTORY: TemplateIdentityFactory = Object.freeze({
  create: (kind: TemplateIdentityKind, source: string) => randomIdentity(kind, source),
})
...
export function remapTemplatePageIdentity(
  seed: ProjectPage,
  pageId: string,
  factory: TemplateIdentityFactory = DEFAULT_TEMPLATE_IDENTITY_FACTORY,
): RemappedTemplatePage {
