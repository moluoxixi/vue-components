...
export function syncGiteeMetadata(
  ...
  return collectValidateAndWrite({
    assertSnapshot: snapshot => assertGiteeMetadataSnapshot(snapshot, expectation),
    collectSnapshot,
    outputPath,
  })
}
...
export function formatGiteeSyncError(error: unknown, token?: string): string {
  return formatRepositorySyncError(error, token, true)
}
