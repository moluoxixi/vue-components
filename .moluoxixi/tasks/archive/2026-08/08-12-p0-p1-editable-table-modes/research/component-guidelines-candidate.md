...
- Slot scopes expose effective `mode`, optional `rowId`, row/column context, and scoped mode actions. `rowId` and row/cell actions are available when stable row identity is configured.
...
- Every override-state mutation emits one typed `modeChange`; setting the same stored override or clearing an absent override emits nothing. A lower-priority override may emit with equal `previousMode` and `mode` when a higher-priority override masks its effective result. Bulk changes emit once with the number of overrides cleared.
...
| Row/cell action without stable row identity | Warn and leave mode state unchanged; never key by row index |
...
- Assert single payload fields (`scope`, `action`, `mode`, `previousMode`, `rowId`, `columnId` as applicable), bulk payload fields (`scope`, `action`, `cleared`, `mode`), no-op silence, and one event per bulk action.
