...
规则 JSON 是稳定协议。外部输入先通过 `parseRuleSet()` 严格解析，再使用 `compileRules()` 编译为 Zod schema 和跨字段/custom validator。`rulesToZod()` 只编译单字段 Zod 能力。字段是否必填由 ConfigForm 字段合同独立表达，不属于通用 RuleSet。
...
const parsed = parseRuleSet({
  version: 2,
  base: { type: 'string' },
  rules: [{ kind: 'email', message: '邮箱格式不正确' }],
})
...
支持的基础类型包括 string、number、boolean、date、enum 和 JSON primitive literal。内置规则覆盖字符串长度、regex、email、URL、UUID、数值范围、integer、finite、multipleOf、日期范围、基础跨字段比较和命名 custom validator。
