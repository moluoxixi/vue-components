...
PRD 确定交付形态为带实时预览的 **Web 调试台**，AI 推理与 embedding 调用经 `coderelay.cn` 代理访问外部大模型，密钥存于 gitignored `.env.local`。同时本功能要**对外发布 npm 包**。

关键矛盾：浏览器是不可信前端环境。若让 `ui` 直接携带密钥调外部模型，密钥会被打进前端产物或在网络请求中暴露——发布到 npm 后，任何安装者都能从前端代码或 DevTools 提取密钥并盗用，造成账单与数据风险。

## 决策

**引入 BFF（Backend for Frontend）服务层，由它持有密钥并代理所有外部模型 / embedding 调用；浏览器端 `ui` 零密钥，只与同源 BFF 通信。**

- 密钥仅在运行 BFF 的进程环境读取（`.env.local` / 环境变量注入），永不下发到浏览器。
- BFF 暴露 `/api/query`（NL 查询，串 retriever→ai，流式返回）、`/api/index`（触发/查询索引状态）等接口。
...
- `ui` 依赖方向收敛：禁止直接 import `ai`/`retriever`/`indexer`，只能走 HTTP。
