...
ConfigForm 的物料无关设计器核心。当前版本提供物料注册、SurfaceGraph v2 编辑、画布、物料面板和属性面板，用于编辑结构、布局、静态属性、默认值、静态 options、字段 Required 与 RuleSet v2 通用校验，不内置 Element Plus 或 Ant Design Vue 物料。Designer 只负责聚焦一个 Surface 的作者体验；项目资产、Experience 会话、持久化和 Source 命令由 Studio 组合根负责。完整定位见 [ConfigForm 产品边界](../PRODUCT.md)，当前与目标状态见 [路线图](../ROADMAP.md)。
