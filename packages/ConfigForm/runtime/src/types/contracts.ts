import type { ConfigFormNodeValueScope } from '@moluoxixi/config-form-headless'
...
export interface ComponentNodeConfig {
  ...
  /** Natural object/array value ownership for this container subtree. */
...
export interface ResolvedComponentNode extends Omit<NormalizedNodeConfig, 'slots'> {
  props: Record<string, unknown>
  /** 字符串 registry key 解析后的运行时元数据，供 adapter 保留调用方声明的 key。 */
